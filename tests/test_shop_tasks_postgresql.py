...
def hold_first_task_admission_lock(
    connection,
    cursor,
    statement,
    parameters,
    ...
    while not lock_release_marker.exists():
        if time.monotonic() >= deadline:
            raise TimeoutError("shop task lock release timed out")
        time.sleep(0.02)
...
with app_module.app.app_context():
    event.listen(
        ...
        hold_first_task_admission_lock,
    )


def submit_task(title):
...
try:
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        status_codes = list(executor.map(submit_task, task_titles))
    result_path.write_text(json.dumps({"status_codes": status_codes}))
...
def _wait_for_marker(marker_path, process):
    ...
    while time.monotonic() < deadline:
        ...
        if process.poll() is not None:
            pytest.fail("Task worker exited before acquiring Dataset lock.")
        time.sleep(0.05)
    pytest.fail("Task worker did not acquire the Dataset lock.")


def _wait_for_dataset_lock_waiter(engine, application_name):
    deadline = time.monotonic() + 20
    while time.monotonic() < deadline:
        ...
        if waiting_count >= 1:
            return
        time.sleep(0.05)
    pytest.fail("Second Task request did not wait on Dataset row lock.")


def test_concurrent_task_posts_never_exceed_dataset_limit(tmp_path):
    engine = create_engine(POSTGRESQL_TEST_DATABASE_URL, pool_pre_ping=True)
    repository_root = Path(__file__).resolve().parent.parent
    guest_a_id = uuid.uuid4()
    guest_b_id = uuid.uuid4()
    admin_id = uuid.uuid4()
    application_name = f"shop-task-limit-{uuid.uuid4()}"
    lock_acquired_marker = tmp_path / "task-lock-acquired"
    lock_release_marker = tmp_path / "task-lock-release"
    result_path = tmp_path / "task-worker-result.json"
    worker_process = None
    ...
    try:
            ...
            tasks = [
                {
                    ...
                    "title": f"Guest A既存タスク{index}",
                }
                for index in range(99)
            ]
            tasks.extend(
                [
                    {"dataset_id": guest_b_id, "title": "Guest B保護タスク"},
                    {"dataset_id": admin_id, "title": "Admin保護タスク"},
                ]
            )
            connection.execute(ShopTask.__table__.insert(), tasks)
        ...
        assert sorted(result["status_codes"]) == [303, 400]
